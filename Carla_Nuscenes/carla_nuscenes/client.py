...
class Client:
    ...
    def generate_world(self,world_config):
        ...
        # Ensure settings take effect before navigation queries.
        self.world.tick()
    ...
    def _spawn_walkers(self,count,ego_location=None):
        ...
        while len(walkers) < count and attempts < max_attempts:
            ...
            if spawn_location is None or destination is None:
                continue
            if ego_point is not None and spawn_location.distance(ego_point) < 2.0:
                continue
        ...
        if walker_controller_bp is None:
            print("walker controller blueprint not found, skipping walker controllers")
            return walkers
        ...
        self.world.tick()
        for walker in walkers:
            if walker.controller is None:
                continue
            walker.start()
            walker.controller.set_max_speed(random.uniform(1.0, 4.0))
        return walkers
    ...
    def _spawn_configured_walkers(self,walker_configs):
        ...
        for walker in walkers:
            if walker.controller is None:
                continue
            walker.start()
            walker.controller.set_max_speed(random.uniform(1.0, 2.0))
        return walkers
    ...
    def generate_custom_scene_different_pose(self,scene_config):
        ...
        self.walkers = []
        ego_loc = self.ego_vehicle.get_actor().get_location()
        for walker_config in scene_config["walkers"]:
            nav_location = None
            for _ in range(20):
                candidate = self.world.get_random_location_from_navigation()
                if candidate is not None and candidate.distance(ego_loc) < 30.0:
                    nav_location = candidate
                    break
            if nav_location is None:
                nav_location = self.world.get_random_location_from_navigation()
            if nav_location is None:
                print("Could not get navigation location for walker, skipping")
                continue
            wc = {**walker_config, "location": {"x": nav_location.x, "y": nav_location.y, "z": nav_location.z}}
            self.walkers.append(Walker(world=self.world, **wc))
    ...
    def generate_custom_scene(self,scene_config):
        ...
        # self.ego_vehicle = Vehicle(world=self.world,**scene_config["ego_vehicle"])
        ego_bp_name=scene_config["ego_vehicle"]["bp_name"]
        # If location is None (diversity run), use the map's own spawn points
        # instead of hardcoded per-map coordinates from the config.
        ego_loc_cfg = scene_config["ego_vehicle"].get("location")
        if ego_loc_cfg is None:
            ...
            ego_location = dict(ego_loc_cfg)
            ego_rotation = dict(scene_config["ego_vehicle"].get("rotation", {"yaw":0,"pitch":0,"roll":0}))
        self.count+=1
        ego_location["z"]=0.015
        ...
        # Apply ego speed from diversity config (default: -20 = 20% above limit)
        ego_speed_diff = scene_config.get("ego_speed_diff", -20)
        self.trafficmanager.vehicle_percentage_speed_difference(self.ego_vehicle.get_actor(), ego_speed_diff)
