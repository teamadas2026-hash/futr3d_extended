...
class FUTR3D(MVXTwoStageDetector):
    ...
    def extract_pts_feat(self, pts):
        ...
        # Check if points are empty to avoid CUDA errors
        if pts is None or len(pts) == 0 or all(p.shape[0] == 0 for p in pts):
            device = pts[0].device if pts and len(pts) > 0 else torch.device('cuda')
            # FPN has num_outs=4, so return a list of 4 zero feature maps
            # Shape matches pts_neck output: 4 levels at stride 1,2,4,8 of base 180x180
            return [
                torch.zeros((1, 256, 180, 180), device=device),
                torch.zeros((1, 256, 90, 90),   device=device),
                torch.zeros((1, 256, 45, 45),   device=device),
                torch.zeros((1, 256, 23, 23),   device=device),
            ]
    ...
    def extract_radar_feat(self, radar, img_metas):
        ...
        # Check if radar data is empty to avoid CUDA errors
        if radar is None or len(radar) == 0 or all(r.shape[0] == 0 for r in radar):
            # Return empty features with correct shape
            device = radar[0].device if radar and len(radar) > 0 else torch.device('cuda')
            return [torch.zeros((1, 128, 180, 180), device=device)]
    ...
    def voxelize(self, points):
        ...
        for res in points:
            # Skip empty point clouds to avoid CUDA errors
            if res.shape[0] == 0:
                device = res.device
                voxels.append(torch.zeros((0, 10, 5), device=device))
                coors.append(torch.zeros((0, 3), dtype=torch.int32, device=device))
                num_points.append(torch.zeros((0,), dtype=torch.int32, device=device))
                continue
        ...
        # Handle case where all point clouds are empty
        if len(voxels) == 0 or all(v.shape[0] == 0 for v in voxels):
            device = points[0].device if points and len(points) > 0 else torch.device('cuda')
            return (torch.zeros((0, 10, 5), device=device),
