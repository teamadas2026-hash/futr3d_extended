...
class LoadRadarPointsMultiSweeps(object):
    ...
    def _load_points(self, pts_filename):
        ...
        try:
            ...
            # CARLA radar files in this repo can be stored as raw float32
            # tensors with .pcd suffix. Fall back to binary parsing.
            points = np.fromfile(pts_filename, dtype=np.float32)
            if points.size == 0:
                return points.reshape(0, self.load_dim).astype(np.float32)
            if points.size % self.load_dim == 0:
                return points.reshape(-1, self.load_dim).astype(np.float32)

            # Some CARLA exports use a different raw column width than
            # self.load_dim. Infer a valid width, then pad/truncate columns.
            inferred_dim = None
            candidate_dims = [20, 19, 18, 16, 12, 10, 8, 6, 5, 4]
            ...
            if inferred_dim is None:
                raise ValueError(
                    f'Invalid radar file shape for {pts_filename}: '
                    f'{points.size} values cannot be reshaped to known dims '
                    f'or configured load_dim={self.load_dim}')

            points = points.reshape(-1, inferred_dim)
            if inferred_dim < self.load_dim:
                pad = np.zeros((points.shape[0], self.load_dim - inferred_dim),
                               dtype=np.float32)
                points = np.concatenate([points, pad], axis=1)
            elif inferred_dim > self.load_dim:
